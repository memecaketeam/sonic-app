...
export interface TransferNotificationContentProps {
  id: string;
}

export const TransferNotificationContent: React.FC<
  TransferNotificationContentProps
> = ({ id }) => {
  const dispatch = useAppDispatch();

  const { addNotification, popNotification } = useNotificationStore(),
    { getBalances } = useBalances();
  ...
  var batchData = useTransferBatch({
    amount: value,
    token: selectedToken,
    address: toAddress,
  ...
  const handleStateChange = () => {
    ...
    if (batch?.state) {
      if (
        Object.values(TransferModalDataStep).includes(
          batch?.state as TransferModalDataStep
        )
      ) {
        dispatch(
          modalsSliceActions.setTransferModalData({
            step: batch?.state as TransferModalDataStep,
          })
        );
      }
    }
  ...
  const handleError = (err?: any) => {
    ...
    addNotification({
      title: `Transfer ${value} ${selectedToken?.symbol} failed`,
  ...
  useEffect(() => {
    handleOpenModal();
    if (typeof batchExecutalbe?.execute === 'undefined' || !batch?.state)
      return;
      ...
      batchExecutalbe
        ...
        .then((data: any) => {
          if (data) {
            ...
            getBalances();
            addNotification({
              title: `Transfered ${value} ${selectedToken?.symbol}`,
