...
export const useAddLiquidityBatch = (addLiquidityParams: AddLiquidity) => {
  ...
  var token0Amt = parseFloat(
    deposit0Params?.amount ? deposit0Params?.amount : '0'
  );
  var token1Amt = parseFloat(
    deposit1Params?.amount ? deposit1Params?.amount : '0'
  );
  ...
  if (token0Type == 'icrc1' || token1Type == 'icrc1') {
    getICRCAcnt = intitICRCTokenDepositIn();
    steps = [...steps, 'getacnt'];
  }
  ...
  if (token0Amt > 0) {
    if (token0Type == 'dip20' || token0Type == 'yc') {
      ...
      approve0 = useICRCTransferMemo({
        ...deposit0Params,
        tokenAcnt: getICRCAcnt,
      });
      deposit0 = useDepositTransactionMemo(deposit0Params);
  ...
  const openBatchModal = () => {
    dispatch(
      modalsSliceActions.setAddLiquidityModalData({
        ...
        fromValue: addLiquidityParams.token0.value,
        toValue: addLiquidityParams.token1.value,
        batchTrx: LiquidityBatchTx,
      })
    );
